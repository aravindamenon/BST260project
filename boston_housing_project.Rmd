...
We wanted to examine how the real estate prices in and around Boston were affected in the years before and after the start of the COVID-19 pandemic defined as March 2020 when Mayor Marty Walsh decladed a [public health emergency](https://www.boston.gov/news/coronavirus-disease-covid-19-boston).  Given that Boston is a huge healthcare hub with some of the leading hospitals, pharmaceutical, and biotech companies, its housing market is more likely to be impacted by COVID-19 trends than other cities. Thus, it is a good model city to study the pandemic-related factors affecting housing inflation. Furthermore, given that Boston has the 4th most number of colleges in the US, trends in housing markets during the COVID-19 pandemic were likely to be affected by the remote learning of 2020 and 2021 influx of college and graduate students as schools moved toward hybrid models of remote and in-person learning.
 
In addition to the effect of the pandemic, we also wanted to explore other factors that may have affected the property prices. We want to contrast how price inflation varies according to neighbourhoods and differences in neighborhood area deprivation index (ADI), school ratings, air quality index (AQI), etc. if available. Thus, we will begin to understand the local trends in housing prices in Boston before and after the start of the COVID-19 pandemic and begin to explore COVID-related and unrelated factors that could have influenced these trends.

# Related work

We were in part inspired by this article on national housing trends during the COVID-19 pandemic that was released by [the White House](https://www.whitehouse.gov/cea/written-materials/2021/09/09/housing-prices-and-inflation/).
...
* Objective 2: Utilizing regression models based on property data from pre-pandemic (2017-2019) and pandemic (2020-2021) years, we will predict the possible change in housing prices for different metropolitan and suburban regions of Boston in 2022 given 1) there is no further COVID-19 surge and 2) there is another COVID-19 surge.
 
* Objective 3: To assess for factors that affect inflation of property price beyond COVID-19, including sociodemographic (i.e., neighbourhood, [ADI](https://www.neighborhoodatlas.medicine.wisc.edu/), facilities (i.e., [proximity to medical center](https://data.boston.gov/dataset/hospital-locations), [school district ratings](https://www.niche.com/k12/search/best-school-districts/t/boston-suffolk-ma/) and environmental data (i.e., AQI).

# Data sources

1. Primary data regarding the actual housing values was defined as government assessment values and obtained through the [city of Boston property assessment database](https://data.boston.gov/dataset/property-assessment) for the years of 2017-2021.
 
2. Real world property closing price data was obtained through web scraping from publicly available property listing websites including:
* For pre-COVID housing sale data from 2015-2019, average housing sale prices stratified by metropolitan Boston neighborhoods were obtained from [Boston Magazine](https://www.bostonmagazine.com/).
  + Years [2015-2016](https://www.bostonmagazine.com/best-places-to-live-2017-single-family-homes/)
  + Years [2017-2018](https://www.bostonmagazine.com/top-places-to-live-2019-single-family-homes/)
  + Years [2018-2019](https://www.bostonmagazine.com/property/single-family-home-prices/)
* For post-COVID housing sale data from 2020, average housing sale price stratified by neighborhood was obtained from [Redfin](https://www.redfin.com/city/1826/MA/Boston/housing-market).
* For post-COVID housing sale data from 2021, housing sale prices from the 5 most recent properties in each metro Boston Neighborhood was individually scrapped from [Redfin](https://www.redfin.com/city/1826/MA/Boston/recently-sold).
...
*Pre-analysis Planning Adjustments*

After discussion with our TA, Rolando, we refined our questions to:
...
*Post-web scraping/data uploading Adjustments*

The initial plan was to scrape individual housing sale data from each year and to match the property with the governement assessments data but adjustments were made after encountering obstacles in data scraping and merging of the primary data.

* Historic hosuing sale data (i.e., for 2015-2020) at the individual property level is not available on webistes such as Redfin, Zillow, and Trulia

* Historic sale price data was only avaialble as an average for city or neighborhoods within the city

* Average property assessment value by neighborhood for 2015-2020 were used since there was not individual property sale data available for those years

* Sale prices from 2021 would have to be scraped for each individual property and since our coding skills were not advanced enough to automate this process, it limited data collection to 5 properties from each of the approximately 20 Boston neighborhoods

* Linking of the 2021 assessments data and the actual housing sale data proved extremely difficult because there was not a unifying ID, therefore average property assessment value by neighborhoods was used
 
*Analysis Adjustments*

The final dataset was extremely large but there was significant missingness for several covariables. To account for this and to prevent maxing out on R memory, the analysis was restricted to data from 2017-2021 although 2015 and 2016 housing sale data had been found.
...
We looked at property taxes across neighborhoods and years to get a sense of general trends before incorporating sales data.
...
avgptax<-eval_all1%>%filter(neighborhood!='NA')%>%ggplot(aes(x=year,y=meantax,color=neighborhood))+geom_line()+geom_point()+ylim(2500,10000)+theme_bw()+ggtitle("Trend in mean property tax across neighbourhoods")+xlab("Year")+ylab("Average Property Tax")
avgptax
#Wide variance and limiting outliers
taxval<-aov(meantax~neighborhood+year,data=eval_all1)
summary(taxval)
```

We also looked at assessed property values (per square foot) across neighborhoods and years to get a sense of general trends before incorporating sales data.
...
prsqftpictax<-eval_all2%>%ggplot(aes(x=year,y=avgprsqft,color=neighborhood))+geom_line()+geom_point()+ylim(NA,1000)+theme_bw()+ggtitle("Trend in mean price per square foot across neighbourhoods")+xlab("Year")+ylab("Average price per sq feet")
prsqftpictax
...
We used 2020 Redfin data to compare sale price per square foot from the real market to the assessed value per square foot (found in `6.2.3 More exploratory analysis` above).
...
## Cumulative data for analysis

Steps included:

* Joining sales data scraped from Redfin and Boston magazine with the City of Boston data

* Removed missing values from key variables (particularly outcome), making the assumption that missingness is random and additional bias would not be introduced.

* Created a binary variable "COVID19" that was `FALSE` for years 2017-19 and `TRUE` for years 2020-21.

* Created a binary variable "SALE" that was `TRUE` for high sale values, or above the median of average sale price, and `FALSE` for low sale values, or below the median of average sale price)
...
We also found at this point that we were using a lot of memory in the RStudio global environment. For some of us, the analysis below simply did not run ("Error: vector memory exhausted (limit reached?)"). To address this issue, we pursued two steps:

1. Clearing dataframes and lists that are no longer needed in the rest of this R markdown.

2. Taking a random sample of the cumulative data and using that for the rest of the analysis. This is certainly a limitation to our analysis, but we agreed that a random sample of 750,000 still gave us adequate power for our analysis. **Of note,** we also ran our models on the full data set (>9 million, called “full_data_new”), 25% of the data, and 750,000 observations of the data (“rando”). We did not include that code in this R markdown, and opted to show the models based on the sample size of 750,000 observations for consistent results, as we found that the larger samples would often prevent the file from knitting or running, or would exhaust the vector memory. We've included some commentary on our analysis run on the full data or 25% of the data, as applicable.

```{r}
#Clear unneeded data frames to clear memory
rm(eval_all, eval_all1, eval_all2, eval_all2020, eval2017, eval2018, eval2019, eval2020, eval2021, alls, avgptax, bbay, beachill, bos, bos.list, bright, brook, camb, chesthill, col_order, col_order2, combined2020, condo_nineteen, condo_sixteen, condo_twenty, ct, ctown, df.2021sales, df.all.sales, df.bos.housing, df.boston, df.boston1, dorch, dt, ebos, funoutput.boston, house_nineteen, house_sixteen, house_twenty, hpark, inflation, jp, ken, l_condo_nineteen, l_condo_sixteen, l_condo_twenty, l_house_nineteen, l_house_sixteen, l_house_twenty, matta, nend, prsqftpictax, ros, rownames, rox, sbos, sea, send, somer, t_condo_nineteen, t_condo_sixteen, t_condo_twenty, t_house_nineteen, t_house_sixteen, t_house_twenty, taxval, val, wrox)

#Taking a random sampling of our data (without replacement) for analysis
set.seed(1)
rando <- sample_n(full_long_new, 750000) 
```

# Visualization

## Home prices by Boston neighborhood

We observe that there is an overall spike in home sale prices in 2020 in some neighborhoods, the onset year of the pandemic, and that this seems to decrease almost back to expected levels in 2021. 
...
sale_long <- rando %>% group_by(neighborhood, year.x, type) %>% summarize(Sale_Total = mean(avg_sale_price)) %>% ungroup()
...
## Property assessment by Boston neighborhood

There does not appear to be any discernible shift in property assessment values, regardless of the pandemic. 
...
Since above we had introduced a new (binary) variable "COVID19" (explained above in `6.6 Cumulative data for analysis`), a multiple linear regression looking at neighborhood (as factor) and COVID19 (as binary) was used to provide an *overall* estimate for the price change during the pandemic in a home's sale price.
...
The context of COVID appeared to increase a home's sale price as compared to pre-COVID times, neighborhood being held constant. For computation power and speed in this file we included a random draw. This said, the full data set with home sale price ("full_long_new") increased \$1,446,869 in the context of COVID with 95% confidence interval [1,435,742, 1,457,996], holding neighborhood constant (analysis not shown). In the random ~10% sample ("rando") with wider 95% CI, the effect estimate is an increased home sale price of \$1,438,942 increase [1,399,017, 1,478,867], holding neighborhood constant. We noted here that the 10% sample is a reasonable proxy for the full data frame. We will utilize the random sample to fit a logistic regression model with a training and test set later. For now, we should examine the years (each of 2020 and 2021) for a more nuanced picture as below.

To allow for a bit finer resolution and taking into account the visualizations above that appear to show a return back down in homes' sale prices in 2021, we regressed neighborhood and year as factors for the outcome of average sale price of a home. This allowed for rough estimation of the factor of 2020, as compared to 2017, and then of 2021 as compared to 2017 (and indirectly to 2020), assuming the same neighborhood.
...
We moved on to take advantage of our recently created binary variable "SALE" (explained above in `6.6 Cumulative data for analysis`) to fit a GLM model. Given the large number of observations, we had the luxury of using 70% of our data to train and 30% to test for the outcome of high vs low expected sale price. The overall accuracy for a given random sample ranges in the 70s%, sensitivity of 90s% and specificity of 30-40s%. 
...
Step 1: We initially built a training and test set from 2017-2019
...
Step 2: Decision tree built on the training set collected from 2017-2019 and applied on the test set from 2017-2019.
...
Even when limited to 2017-2019, where COVID-19 would not be a factor, this was not a great model, since it only contains 4 terminal leaf nodes that were determined by two big groups of neighborhoods and a further splitting of condo vs house.

We went back to Step 1 and tweaked our model to instead try to build a decision tree using all data from 2017-2019 as the training set, and then applying it to a test set consisting of all data from 2020. We thought that we could compare the predicted values from this model to the actual sales from 2020, and that this comparison might give us an idea of the effect of COVID-19 on prices in 2020.

Step 1: Training set from all data from 2017-2019
...
Step 2: Applied on all data from 2020
...
This is still quite a poor model. We opted to use the regression models instead.
