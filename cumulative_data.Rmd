...
rm(eval2017, eval2018, eval2019, eval2020, eval2021)
```
...
rm(eval2017res, eval2018res, eval2019res, eval2020res, eval2021res) #deleting dfs we won't use going forward
```
...
df.all.sales <- subset(df.all.sales, select=-c(x1, address, sqr_ft, bedrooms, baths, walk_score, school_district)) #Anticipating what we'll need and keeping data size manageable.
```
...
full_long_new <- subset(full_long_new, select=-c(price_per_sq_ft, number_sold, pid, zipid, st_name, ac_type, heat_type, bedrooms, bath_full, bath_half, condition, year.y))
...
tree_fit <- train_set %>% 
 rpart(avg_sale_price ~ av_bldg + av_land + av_total + gross_tax + land_sf + living_area + yr_built + neighborhood, data = ., maxdepth = 5, minbucket = 1)
