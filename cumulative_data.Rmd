...
p <- full_long_new %>% mutate(neighborhood = as.factor(neighborhood))
summary(p$neighborhood) #confirming several lowr end outliers as before 
...
full_long_new <- full_long_new %>% filter(neighborhood !="Bay Village South End")
full_long_new <- full_long_new %>% filter(neighborhood !="Citywide")
full_long_new <- full_long_new %>% filter(neighborhood !="Somerville")
full_long_new <- full_long_new %>% filter(neighborhood !="Cambridge")
full_long_new <- full_long_new %>% mutate(yr_built=na_if(yr_built,0))
p <- full_long_new %>% mutate(neighborhood = as.factor(neighborhood))
summary(p$neighborhood)
```